...
describe('formatOutput', () => {
    describe('Case 1: Single file, DEFAULT (minify=true, toJson=true, no cache)', () => {
        test('should return raw minified JSON content without wrapper', () => {
            const files: ProcessedFile[] = [{ file: 'test.json', content: { user: { name: 'John' }, age: 30 }, cached: false, originalSize:0, newSize: 0 }];
        ...
        test('should handle plaintext content converted to JSON', () => {
            const files: ProcessedFile[] = [{ file: 'test.txt', content: 'This is minified text content.', cached: false, originalSize:0, newSize: 0 }];
            const options: ReadMinifiedOptions = { minify: true, toJson: true, cache: false, overwrite: false, noOutput: false };
            const output = formatOutput(files, options);
            expect(output).toBe(JSON.stringify('This is minified text content.'));
        });
        test('should handle error in single file without cache', () => {
            const files: ProcessedFile[] = [{ file: 'test.json', error: 'File not found', cached: false, originalSize:0, newSize: 0 }];
            const options: ReadMinifiedOptions = { minify: true, toJson: true, cache: false, overwrite: false, noOutput: false };
            const output = formatOutput(files, options);
    ...
    describe('Case 3: Single file, --no-to-json', () => {
        test('should return minified native format without wrapper', () => {
            const files: ProcessedFile[] = [{ file: 'test.txt', content: 'Minified text content', cached: false, originalSize:0, newSize: 0 }];
    ...
    describe('Case 4: Single file, --no-minify --no-to-json', () => {
        test('should return original content without wrapper', () => {
            const files: ProcessedFile[] = [{ file: 'test.txt', content: 'Original text content.', cached: false, originalSize:0, newSize: 0 }];
    ...
    describe('Case 5: Single file, --cache', () => {
        test('should return JSON wrapper with minified content and cache info', () => {
            const files: ProcessedFile[] = [{ file: 'test.json', content: { user: { name: 'John' } }, cached: true, cachedPath: 'test.compact.json', originalSize:0, newSize: 0 }];
        ...
        test('should handle cache with no cache path when caching failed', () => {
            const files: ProcessedFile[] = [{ file: 'test.json', content: { data: 'value' }, cached: false, originalSize:0, newSize: 0 }];
            const options: ReadMinifiedOptions = { minify: true, toJson: true, cache: true, overwrite: false, noOutput: false };
            const output = formatOutput(files, options);
            const parsed = JSON.parse(output);
        ...
        test('should include error in cache wrapper if present', () => {
            const files: ProcessedFile[] = [{ file: 'test.json', error: 'Parse error', cached: false, originalSize:0, newSize: 0 }];
            const options: ReadMinifiedOptions = { minify: true, toJson: true, cache: true, overwrite: false, noOutput: false };
            const output = formatOutput(files, options);
            const parsed = JSON.parse(output);
    ...
    describe('Case 6: Multiple files, DEFAULT', () => {
        test('should return NDJSON with raw minified content per line', () => {
            const files: ProcessedFile[] = [{ file: 'file1.json', content: { data: 'value1' }, cached: false, originalSize:0, newSize: 0 }, { file: 'file2.json', content: { data: 'value2' }, cached: false, originalSize:0, newSize: 0 }];
            const options: ReadMinifiedOptions = { minify: true, toJson: true, cache: false, overwrite: false, noOutput: false };
            const output = formatOutput(files, options);
        ...
        test('should handle mixed file types in batch', () => {
            const files: ProcessedFile[] = [{ file: 'file1.json', content: { data: 'value' }, cached: false, originalSize:0, newSize: 0 }, { file: 'file2.txt', content: 'plaintext', cached: false, originalSize:0, newSize: 0 }];
            const options: ReadMinifiedOptions = { minify: true, toJson: true, cache: false, overwrite: false, noOutput: false };
            const output = formatOutput(files, options);
            const lines = output.split('\n');
            expect(lines).toHaveLength(2);
        ...
        test('should handle errors in batch without wrapper', () => {
            const files: ProcessedFile[] = [{ file: 'file1.json', content: { data: 'value' }, cached: false, originalSize:0, newSize: 0 }, { file: 'file2.json', error: 'Not found', cached: false, originalSize:0, newSize: 0 }];
            const options: ReadMinifiedOptions = { minify: true, toJson: true, cache: false, overwrite: false, noOutput: false };
            const output = formatOutput(files, options);
            const lines = output.split('\n');
            expect(JSON.parse(lines[0])).toEqual({ data: 'value' });
    ...
    describe('Case 7: Multiple files, --cache', () => {
        test('should return NDJSON with file metadata and cache info per line', () => {
            const files: ProcessedFile[] = [{ file: 'file1.json', content: { data: 'value1' }, cached: true, cachedPath: 'file1.compact.json', originalSize:0, newSize: 0 }, { file: 'file2.json', content: { data: 'value2' }, cached: true, cachedPath: 'file2.compact.json', originalSize:0, newSize: 0 }];
            const options: ReadMinifiedOptions = { minify: true, toJson: true, cache: true, overwrite: false, noOutput: false };
            const output = formatOutput(files, options);
            const lines = output.split('\n');
            expect(lines).toHaveLength(2);
    ...
    describe('Case 8: --no-output (Manifest mode)', () => {
        test('should return manifest for single file', () => {
            const files: ProcessedFile[] = [{ file: 'file1.json', cached: true, cachedPath: 'file1.compact.json', originalSize:0, newSize: 0 }];
            const options: ReadMinifiedOptions = { minify: true, toJson: true, cache: true, overwrite: false, noOutput: true };
            const output = formatOutput(files, options);
            const parsed = JSON.parse(output);
            expect(parsed).toHaveProperty('cached_files');
            expect(parsed).toHaveProperty('cached_file_count', 1);
            expect(parsed.cached_files[0]).toHaveProperty('original_filepath', 'file1.json');
            expect(parsed.cached_files[0]).toHaveProperty('cached_filepath', 'file1.compact.json');
        });
        test('should return manifest for multiple files', () => {
            const files: ProcessedFile[] = [{ file: 'file1.json', cached: true, cachedPath: 'file1.compact.json', originalSize:0, newSize: 0 }, { file: 'file2.json', cached: false, originalSize:0, newSize: 0 }, { file: 'file3.json', cached: true, cachedPath: 'file3.compact.json', originalSize:0, newSize: 0 }];
            const options: ReadMinifiedOptions = { minify: true, toJson: true, cache: true, overwrite: false, noOutput: true };
            const output = formatOutput(files, options);
            const parsed = JSON.parse(output);
            expect(parsed.cached_file_count).toBe(3);
            expect(parsed.cached_files[0].original_filepath).toBe('file1.json');
            expect(parsed.cached_files[1].original_filepath).toBe('file2.json');
            expect(parsed.cached_files[2].original_filepath).toBe('file3.json');
        });
        test('should exclude error field from manifest when not present', () => {
            const files: ProcessedFile[] = [{ file: 'file1.json', cached: true, cachedPath: 'file1.compact.json', originalSize:0, newSize: 0 }, { file: 'file2.json', error: 'Not found', cached: false, originalSize:0, newSize: 0 }];
            const options: ReadMinifiedOptions = { minify: true, toJson: true, cache: false, overwrite: false, noOutput: true };
            const output = formatOutput(files, options);
            const parsed = JSON.parse(output);
            const withoutError = parsed.cached_files[0];
            expect('error' in withoutError).toBeFalsy();
        });
    });
    describe('Edge cases', () => {
        test('should handle empty content object', () => {
            const files: ProcessedFile[] = [{ file: 'empty.json', content: {}, cached: false, originalSize:0, newSize: 0 }];
            const options: ReadMinifiedOptions = { minify: true, toJson: true, cache: false, overwrite: false, noOutput: false };
            const output = formatOutput(files, options);
            expect(output).toBe('{}');
        });
        test('should handle null content', () => {
            const files: ProcessedFile[] = [{ file: 'null.json', content: null, cached: false, originalSize:0, newSize: 0 }];
            const options: ReadMinifiedOptions = { minify: true, toJson: true, cache: false, overwrite: false, noOutput: false };
            const output = formatOutput(files, options);
            expect(output).toBe('null');
        });
        test('should handle array content', () => {
            const files: ProcessedFile[] = [{ file: 'array.json', content: [1, 2, 3], cached: false, originalSize:0, newSize: 0 }];
            const options: ReadMinifiedOptions = { minify: true, toJson: true, cache: false, overwrite: false, noOutput: false };
            const output = formatOutput(files, options);
