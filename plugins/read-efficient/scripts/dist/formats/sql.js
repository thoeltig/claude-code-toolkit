...
function formatSql(rawContent, options) {
    try {
        ...
        const result = grouped.map(group => {
            if (group.action === 'INSERT') {
                ...
                if (group.updates)
                    output.updates = group.updates;
                if (group.where)
                    output.where = group.where;
                return output;
            }
                ...
                if (group.objectType === 'INDEX') {
                    if (group.indexName)
                        output.indexName = group.indexName;
                    if (group.table)
                        output.table = group.table;
                    if (group.columns)
                        output.columns = group.columns;
                    if (group.unique)
                        output.unique = group.unique;
                    if (group.ifNotExists)
                        output.ifNotExists = group.ifNotExists;
                }
                else if (group.objectType === 'VIEW') {
                    if (group.viewName)
                        output.viewName = group.viewName;
                    if (group.orReplace)
                        output.orReplace = group.orReplace;
                    if (group.ifNotExists)
                        output.ifNotExists = group.ifNotExists;
                }
                else {
                ...
                if (group.columns)
                    output.columns = group.columns;
                if (group.columnAliases)
                    output.columnAliases = group.columnAliases;
                if (group.where)
                    output.where = group.where;
                if (group.groupByColumns)
                    output.groupByColumns = group.groupByColumns;
                if (group.havingClause)
                    output.havingClause = group.havingClause;
                if (group.joins)
                    output.joins = group.joins;
                if (group.unionType)
                    output.unionType = group.unionType;
                if (group.caseStatements)
                    output.caseStatements = group.caseStatements;
                if (group.unparsedContent)
                    output.unparsedContent = group.unparsedContent;
                return output;
            }
                ...
                if (group.objectType)
                    output.objectType = group.objectType;
                if (group.objectName)
                    output.objectName = group.objectName;
                if (group.table)
                    output.table = group.table;
                if (group.ifExists)
                    output.ifExists = group.ifExists;
                if (group.cascade)
                    output.cascade = group.cascade;
                if (group.restrict)
                    output.restrict = group.restrict;
                return output;
            }
                ...
                if (group.toSavepoint)
                    output.toSavepoint = group.toSavepoint;
                if (group.savepointName)
                    output.savepointName = group.savepointName;
                return output;
            }
                ...
                if (group.privileges)
                    output.privileges = group.privileges;
                if (group.objectType)
                    output.objectType = group.objectType;
                if (group.objectName)
                    output.objectName = group.objectName;
                if (group.grantee)
                    output.grantee = group.grantee;
                if (group.grantOption)
                    output.grantOption = group.grantOption;
                return output;
            }
                ...
                if (group.alterationType)
                    output.alterationType = group.alterationType;
                if (group.columnDefinition)
                    output.columnDefinition = group.columnDefinition;
                if (group.columnName)
                    output.columnName = group.columnName;
                if (group.newColumnName)
                    output.newColumnName = group.newColumnName;
                if (group.constraintName)
                    output.constraintName = group.constraintName;
                if (group.newName)
                    output.newName = group.newName;
                if (group.cascade)
                    output.cascade = group.cascade;
                if (group.restrict)
                    output.restrict = group.restrict;
...
function detectStatementType(sql) {
    ...
    if (trimmed.startsWith('WITH'))
        return 'SELECT'; // CTE (Common Table Expression) - treat as SELECT
    ...
    if (/^COMMIT/i.test(trimmed))
        return 'COMMIT';
    if (/^ROLLBACK\s+TO/i.test(trimmed))
        return 'ROLLBACK';
    if (/^ROLLBACK/i.test(trimmed))
        return 'ROLLBACK';
    if (/^SAVEPOINT/i.test(trimmed))
        return 'SAVEPOINT';
    if (/^RELEASE\s+SAVEPOINT/i.test(trimmed))
        return 'RELEASE';
    if (/^GRANT/i.test(trimmed))
        return 'GRANT';
    if (/^REVOKE/i.test(trimmed))
        return 'REVOKE';
    return 'UNKNOWN';
}
        ...
        if (columnStr !== '*') {
            ...
            for (const part of columnParts) {
                ...
                if (trimmed.length === 0 || trimmed.includes('('))
                    continue; // Exclude functions for now
...
function parseJoins(sql) {
    ...
    while ((match = joinPattern.exec(sql)) !== null) {
        ...
        if (match[1]) {
            ...
            if (typeStr.includes('LEFT'))
                joinType = 'LEFT';
            else if (typeStr.includes('RIGHT'))
                joinType = 'RIGHT';
            else if (typeStr.includes('FULL'))
                joinType = 'FULL OUTER';
            else if (typeStr.includes('CROSS'))
                joinType = 'CROSS';
            else if (typeStr === 'JOIN')
                joinType = 'INNER'; // Explicit or implicit
        }
...
function parseCaseStatements(sql) {
    ...
    while ((caseMatch = caseRegex.exec(sql)) !== null) {
        ...
        const whens = [];
        let caseType = 'searched'; // Default to searched
...
function parseCreateIndexStatement(sql) {
    const result = { objectType: 'INDEX' };
    // Check for UNIQUE
    ...
    result.columns = columnsStr.split(',').map((col) => col.trim()).filter((col) => col.length > 0);
    return result;
}
function parseCreateViewStatement(sql) {
    const result = { objectType: 'VIEW' };
...
function parseTruncateStatement(sql) {
    // TRUNCATE TABLE name or TRUNCATE name
...
function parseDropStatement(sql) {
    const result = {};
...
function parseValues(valuesStr, columns) {
    const rows = [];
...
function parseValuesAsArrays(valuesStr) {
    const rows = [];
    const rowStrings = parseValueRows(valuesStr);
    for (const rowStr of rowStrings) {
        const values = parseRowValues(rowStr);
...
function groupByTableAndAction(statements) {
    const result = [];
    for (const stmt of statements) {
        ...
        if (lastGroup && lastGroup.table === stmt.table && lastGroup.action === action && canGroup) {
            ...
            if (action === 'INSERT' && stmt.parsed) {
                if (!lastGroup.columns)
                    lastGroup.columns = stmt.parsed.columns;
                if (!lastGroup.rows)
                    lastGroup.rows = [];
                ...
                if (!lastGroup.updates)
                    lastGroup.updates = stmt.parsed.updates;
                if (!lastGroup.where)
                    lastGroup.where = stmt.parsed.where;
            }
                ...
                if (!lastGroup.columns)
                    lastGroup.columns = stmt.parsed.columns;
                if (!lastGroup.columnAliases)
                    lastGroup.columnAliases = stmt.parsed.columnAliases;
                if (!lastGroup.where)
                    lastGroup.where = stmt.parsed.where;
                if (!lastGroup.groupByColumns)
                    lastGroup.groupByColumns = stmt.parsed.groupByColumns;
                if (!lastGroup.havingClause)
                    lastGroup.havingClause = stmt.parsed.havingClause;
                if (!lastGroup.joins)
                    lastGroup.joins = stmt.parsed.joins;
                if (!lastGroup.unionType)
                    lastGroup.unionType = stmt.parsed.unionType;
                if (!lastGroup.caseStatements)
                    lastGroup.caseStatements = stmt.parsed.caseStatements;
                if (!lastGroup.unparsedContent)
                    lastGroup.unparsedContent = stmt.parsed.unparsedContent;
            }
                ...
                if (!lastGroup.objectType)
                    lastGroup.objectType = stmt.parsed.objectType;
                if (!lastGroup.objectName)
                    lastGroup.objectName = stmt.parsed.objectName;
                if (!lastGroup.ifExists)
                    lastGroup.ifExists = stmt.parsed.ifExists;
                if (!lastGroup.cascade)
                    lastGroup.cascade = stmt.parsed.cascade;
                if (!lastGroup.restrict)
                    lastGroup.restrict = stmt.parsed.restrict;
            }
        }
        else {
            const group = {
            ...
            if (action === 'INSERT' && stmt.parsed) {
                ...
                group.columns = stmt.parsed.columns;
                if (stmt.parsed.columnAliases)
                    group.columnAliases = stmt.parsed.columnAliases;
                group.where = stmt.parsed.where;
                if (stmt.parsed.groupByColumns)
                    group.groupByColumns = stmt.parsed.groupByColumns;
                if (stmt.parsed.havingClause)
                    group.havingClause = stmt.parsed.havingClause;
                if (stmt.parsed.joins)
                    group.joins = stmt.parsed.joins;
                if (stmt.parsed.unionType)
                    group.unionType = stmt.parsed.unionType;
                if (stmt.parsed.caseStatements)
                    group.caseStatements = stmt.parsed.caseStatements;
                if (stmt.parsed.unparsedContent)
                    group.unparsedContent = stmt.parsed.unparsedContent;
            }
            else if (action === 'TRUNCATE' && stmt.parsed) {
                ...
                if (stmt.parsed.objectName)
                    group.objectName = stmt.parsed.objectName;
                if (stmt.parsed.ifExists)
                    group.ifExists = stmt.parsed.ifExists;
                if (stmt.parsed.cascade)
                    group.cascade = stmt.parsed.cascade;
                if (stmt.parsed.restrict)
                    group.restrict = stmt.parsed.restrict;
            }
            else if ((action === 'BEGIN' || action === 'COMMIT' || action === 'ROLLBACK' || action === 'SAVEPOINT' || action === 'RELEASE') && stmt.parsed) {
                if (stmt.parsed.toSavepoint)
                    group.toSavepoint = stmt.parsed.toSavepoint;
                if (stmt.parsed.savepointName)
                    group.savepointName = stmt.parsed.savepointName;
            }
            else if ((action === 'GRANT' || action === 'REVOKE') && stmt.parsed) {
                if (stmt.parsed.privileges)
                    group.privileges = stmt.parsed.privileges;
                if (stmt.parsed.objectType)
                    group.objectType = stmt.parsed.objectType;
                if (stmt.parsed.objectName)
                    group.objectName = stmt.parsed.objectName;
                if (stmt.parsed.grantee)
                    group.grantee = stmt.parsed.grantee;
                if (stmt.parsed.grantOption)
                    group.grantOption = stmt.parsed.grantOption;
                if (stmt.parsed.cascade)
                    group.cascade = stmt.parsed.cascade;
                if (stmt.parsed.restrict)
                    group.restrict = stmt.parsed.restrict;
                ...
                if (stmt.parsed.alterationType)
                    group.alterationType = stmt.parsed.alterationType;
                if (stmt.parsed.columnDefinition)
                    group.columnDefinition = stmt.parsed.columnDefinition;
                if (stmt.parsed.columnName)
                    group.columnName = stmt.parsed.columnName;
                if (stmt.parsed.newColumnName)
                    group.newColumnName = stmt.parsed.newColumnName;
                if (stmt.parsed.constraintName)
                    group.constraintName = stmt.parsed.constraintName;
                if (stmt.parsed.newName)
                    group.newName = stmt.parsed.newName;
                if (stmt.parsed.cascade)
                    group.cascade = stmt.parsed.cascade;
                if (stmt.parsed.restrict)
                    group.restrict = stmt.parsed.restrict;
...
function parseBeginStatement(_sql) {
    return { action: 'BEGIN' };
}
function parseCommitStatement(_sql) {
    return { action: 'COMMIT' };
}
function parseRollbackStatement(sql) {
...
function parseSavepointStatement(sql) {
    const result = { action: 'SAVEPOINT' };
...
function parseReleaseStatement(sql) {
...
function parseGrantStatement(sql) {
    ...
    if (privilegesMatch) {
        ...
        if (privStr.toUpperCase() === 'ALL') {
            ...
            result.privileges = privStr.split(',').map((p) => p.trim().toUpperCase()).filter((p) => p.length > 0);
        }
    }
...
function parseRevokeStatement(sql) {
...
function parseAlterTableStatement(sql) {
    const result = { action: 'ALTER' };
    // ADD COLUMN
...
function areInsertColumnsCompatible(groupColumns, stmtColumns) {
    // If group doesn't have columns yet, it's compatible
    if (!groupColumns)
        return true;
    // If both are '*', compatible
    if (groupColumns === '*' && stmtColumns === '*')
        return true;
    // If both are arrays, compatible (can merge rows with same columns)
    if (Array.isArray(groupColumns) && Array.isArray(stmtColumns))
        return true;
    // Different types: '*' vs array, not compatible
    return false;
}
function transformToTableGrouped(flatStatements) {
    const tableMap = new Map();
    const systemStatements = []; // For statements without a table (transactions, permissions)
    for (const stmt of flatStatements) {
        ...
        if (isSystemStatement) {
            ...
            const action = { action: stmt.action };
