...
export function formatLogs(rawContent: string, options: { minify: boolean }): string {
    try {
        const lines = rawContent.split('\n').filter(line => line.trim().length > 0);

        if (lines.length === 0) return minifyJson([]);
...
function detectLogType(firstLine: string): 'rfc5424' | 'rfc3164' | 'apache' {
    if (!firstLine.startsWith('<')) return 'apache';

    const priorityEnd = firstLine.indexOf('>');
    if (priorityEnd === -1) return 'apache';

    const afterPri = firstLine.substring(priorityEnd + 1);

    if (afterPri.startsWith('1 ')) return 'rfc5424';

    return 'rfc3164';
}

function parseApacheNginxLine(line: string): any {
...
function parseRfc3164Line(line: string): any {
    const row: any = {};
...
function parseRfc5424Line(line: string): any {
    const row: any = {};
...
function parseStructuredData(sdString: string): any {
    const result: any = {};

    const sdMatch = sdString.match(/\[(.*?)\]/g);
    if (!sdMatch) return result;
...
function parseLogFields(line: string): string[] {
    const fields: string[] = [];
    let current = '';
    let inQuotes = false;
